...
const loadFileTypeFromBuffer = (() => {
   let loader: Promise<FileTypeFromBuffer> | undefined;
   return async () => {
      if (!loader) {
         loader = dynamicImport<FileTypeModule>('file-type').then(
...
const loadMime = (() => {
   let loader: Promise<MimeInstance> | undefined;
   return async () => {
      if (!loader) {
         loader = dynamicImport<MimeModule>('mime').then((mod) =>
            hasDefaultExport(mod) ? mod.default : mod,
         );
      }
      return loader;
   };
