...
import type { ConsumeEmailLoginMagicLinkInput } from '../../input';
...
@Injectable()
export class ConsumeEmailLoginMagicLinkService {
   constructor(
   ...
   async run(
      input: ConsumeEmailLoginMagicLinkInput,
      meta: RequestMeta,
      ...
      await this.auditService.run({
         userId: user.id,
         eventType: AuthAuditEventTypeEnum.login_email_magic_consumed,
         channel: input.channel as PrismaAuthChannelEnum,
         metadata: {
