...
export const logConfigValidation = z
   ...
   .refine(
      ...
      {
         message:
            'LOGSENE_TOKEN é obrigatório quando LOG_ENABLE_LOGSENE for true',
         path: ['LOGSENE_TOKEN'],
      },
   )
   .refine(
      ...
      {
         message:
            'Configuração do CloudWatch incompleta. Todos os campos obrigatórios devem ser informados quando LOG_ENABLE_AWS_CLOUD_WATCH for true.',
      },
   );

export function validateLogEnv(env: NodeJS.ProcessEnv) {
   try {
      logConfigValidation.parse(env);
   } catch (error) {
      console.error('Falha na validação da configuração do logger:', error);
      process.exit(1);
   }
}
